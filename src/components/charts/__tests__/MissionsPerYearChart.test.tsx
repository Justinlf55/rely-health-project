...
import MissionsPerYearChart from '../MissionsPerYearChart';
...
describe('MissionsPerYearChart', () => {
  ...
  it('renders with accessible section role and label', async () => {
    render(createElement(DashboardProvider, null, createElement(MissionsPerYearChart)));
    await waitFor(() => {
      expect(screen.getByRole('region', { name: /Missions per year chart/i })).toBeInTheDocument();
    });
  });
  ...
  it('renders sr-only description', async () => {
    render(createElement(DashboardProvider, null, createElement(MissionsPerYearChart)));
    await waitFor(() => {
      expect(screen.getByText(/Bar chart showing missions per year/i)).toBeInTheDocument();
    });
  });
});
